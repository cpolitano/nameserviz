...
// NewHandler returns a handler for "nameserviz" type messages.
...
func handleMsgBuyName(ctx sdk.Context, keeper Keeper, msg MsgBuyName) sdk.Result {
	...
	if keeper.HasOwner(ctx, msg.Name) { // check if name has owner already
		// if yes, former owner receives money from Buyer
		_, err := keeper.coinKeeper.SendCoins(ctx, msg.Buyer, keeper.GetOwner(ctx, msg.Name), msg.Bid)
		if err != nil {
			return sdk.ErrInsufficientCoins("Buyer does not have enough coins").Result()
		}
		// if no current owner, nameserviz burns the coins from Buyer
	} else {
	...
	keeper.SetOwner(ctx, msg.Name, msg.Buyer) // set owner as Buyer
	keeper.SetPrice(ctx, msg.Name, msg.Bid)   // set price as Bid
	return sdk.Result{}
}
